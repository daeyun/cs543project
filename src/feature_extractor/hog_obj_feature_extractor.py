...
class HogObjFeatureExtractor:
    ...
    def compute_features(self, img, rect, container_rect):
        ...
        :return: ndarray of shape (71,) of type float32 containing the following features
        ...
        cc_dissimilarity = np.array([1 / (i / 10 + 1) for i in cc])

        # Location and size
        ls = self.normalize_location_and_size(rect, container_rect)

        feature_vector = np.hstack((hog[:, 0], cc_dissimilarity, ls))
